...
def get_full_guide_text(code):
    ...
    if code == 'TY':
        ...
        text += "🏥 소음인 체질 증상 및 질환\n"
        text += "특성: 신대비소(腎大脾小). 신장/생식기 기능은 좋으나 위장이 차고 소화력이 약함. 몸이 차고(냉증), 예민하며 체력이 약해지기 쉽움.\n"
...
※ 본 결과는 자가진단을 바탕으로 한 참고용이며, 정확한 의학적 진단과 처방은 한의원에 내원하여 상담받으시길 바랍니다.
    ...
    # ----------------------------------
        ...
        # [핵심 로직 변경] 점수 차이에 따른 확신도 및 타이틀 결정
        # 1등과 2등의 점수 차이 계산
        score_diff = sorted_scores[0][1] - sorted_scores[1][1]
        
        # 신뢰도 분석 로직 (0.4점 이상: 확진 / 0.2~0.4: 유력 / 0.2 이하: 복합)
        if score_diff > 0.4:
             my_name = TYPE_MAP[sorted_scores[0][0]]
             st.title(f"🎉 전형적인 [{my_name}] 입니다! (확실)")
             st.success(f"🛡️ **진단 신뢰도: 매우 높음 (확진)**\n\n다른 체질과 점수 차이가 **{score_diff:.1f}점**으로 매우 큽니다. {my_name}의 특성이 뚜렷하게 나타납니다.")
             show_details = True # 상세 내용 표시

        elif score_diff > 0.2:
             my_name = TYPE_MAP[sorted_scores[0][0]]
             st.title(f"🎉 [{my_name}] 성향이 강합니다 (유력)")
             st.info(f"📊 **진단 신뢰도: 높음 (유력)**\n\n2순위 체질보다 **{score_diff:.1f}점** 더 높게 측정되었습니다. {my_name}일 가능성이 매우 높으나, 생활 습관에 따라 다른 성향이 일부 보일 수 있습니다.")
             show_details = True # 상세 내용 표시

        else:
             # 점수 차이가 0.2 이하인 경우 (복합 체질)
             # 근소한 차이인 체질들 모두 찾기
             close_candidates = []
             close_candidates.append(sorted_scores[0])
             for i in range(1, 4):
             ...
             st.title(f"🎉 [{title_text}] 성향이 비슷합니다 (복합)")
             st.warning(f"📢 **진단 신뢰도: 복합 성향 (판단 보류)**\n\n상위 체질 간 점수 차이가 **{score_diff:.1f}점 이내**로 매우 근소합니다.\n두 가지 이상의 성향이 섞여 있거나, 현재 몸 컨디션에 따라 결과가 모호할 수 있습니다.\n\n정확한 진단을 위해 한의원 내원 상담을 권장합니다.")
             show_details = False # 상세 내용 숨김 (복합일 때는 너무 긴 내용보다는 상담 유도)

        # 닥터 제마의 한마디 (수정된 디자인: 어두운 배경)
        st.markdown("""
        <div style="background-color:#101c2e; padding:15px; border-radius:8px; margin-top:20px; color: #cfd8dc;">
            <div style="color: #60a5fa; font-weight: bold; margin-bottom: 5px; font-size: 1.1rem;">
                💡 닥터 제마의 한마디
            </div>
        ...
        # 범주형(Categorical) 타입을 사용하여 정렬 순서를 강제로 고정합니다.
        ...
        # =========================================================
        # 상세 가이드 및 유명인 정보 출력 (show_details 변수에 따라 결정)
        # =========================================================
        if show_details:
            ...
            if my_code == 'TE': # 태음인
                ...
                **💡 평소 생활 실천 사항**
                ...
                st.markdown("""
                **특성:** 신대비소(腎大脾小). 신장/생식기 기능은 좋으나 위장이 차고 소화력이 약함. 몸이 차고(냉증), 예민하며 체력이 약해지기 쉽움.
